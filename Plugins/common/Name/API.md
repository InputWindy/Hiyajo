...
服务层：`FNamePool` 是 `FFrameExtension + IPipeline<IPreInit, IInit, IPostInit, IPreShutdown, IShutdown, IPostShutdown>`（`FNamePool.dll`）。**字符串驻留池**——`FName` 是不可变驻留字符串标识符，构造时把字符串 intern 进全局池，相同字符串共享同一条目，O(1) 比较。默认构造的 `FName` 是 None（空）。
...
| 签名 | 说明 |
|------|------|
| `MAHO_DECLARE_FRAME(FNamePool)` | 层声明宏（DLL 导出入口） |
...
| 签名 | 说明 |
|------|------|
| `MAHO_NAME_API` | 修饰本 DLL 导出的符号（`GetNamePool`、`CreateFrame`） |
