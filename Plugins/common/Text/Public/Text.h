...
// Text ‚Ä?localized text (engine Common, TSingleton). FText handles + FTextManager
...
/**
 * Localized text handle ‚Ä?stores {Namespace, Key, Source}. Resolve() looks up
 ...
 *   FTextManager::Get().AddTranslation("MainMenu", "Title", Culture::Chinese, "‰∏ªËèúÂç?);
 *   FTextManager::Get().AddTranslation("MainMenu", "Title", Culture::Japanese, "„É°„Ç§„É≥„É°„Éã„É•„É?);
 *
 *   const FText Title = FText("MainMenu", "Title", "Main Menu");
 *
 *   FTextManager::Get().SetCulture(std::string(Culture::Chinese));
 *   const std::string Shown = Title.Resolve();   // "‰∏ªËèúÂç?
 */
class FText
{
public:
	...
	/** Resolve against the current culture ‚Ä?falls back to Source when absent. */
...
{
public:
	/** Process-unique accessor ‚Ä?declared here, defined in Text.cpp (in Text.dll). */
