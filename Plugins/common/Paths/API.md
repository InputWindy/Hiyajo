...
服务层：`FPaths` 是 `FLayer<IPreInit, IInit, IPostInit, IPreShutdown, IShutdown, IPostShutdown>`（`FPaths.dll`）。**虚拟路径解析**——把引擎 / 项目根别名（`Engine`、`Game`…）映射到物理路径；`Resolve` 吃 `"Alias/Sub"` 或 `"Alias:Sub"` 形式。
...
| 签名 | 说明 |
|------|------|
| `MAHO_DECLARE_LAYER(FPaths)` | 层声明宏（DLL 导出入口） |
