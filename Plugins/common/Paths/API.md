...
服务层：`FPaths` 是 `FFrameExtension + IPipeline<IPreInit, IInit, IPostInit, IPreShutdown, IShutdown, IPostShutdown>`（`FPaths.dll`）。**虚拟路径解析**——把引擎 / 项目根别名（`Engine`、`Game`…）映射到物理路径；`Resolve` 吃 `"Alias/Sub"` 或 `"Alias:Sub"` 形式。
...
| 签名 | 说明 |
|------|------|
| `MAHO_DECLARE_FRAME(FPaths)` | 层声明宏（DLL 导出入口） |
...
| 签名 | 说明 |
|------|------|
| `MAHO_PATHS_API` | 修饰本 DLL 导出的符号（`GetPaths`、`CreateFrame`） |
