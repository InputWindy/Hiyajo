...
- Implementation notes:
  - FLog is a **service layer** (`FFrameExtension + IPipeline<IPreInit, IInit, IPostInit, IPreShutdown, IShutdown, IPostShutdown>`), not a singleton. `GetLog()` is defined in `Private/Log.cpp` (published at Initialize, cleared at Shutdown).
