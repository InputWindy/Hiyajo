...
FRHITexture* FRHIResourceManager::AcquireTexture(const FRHITextureDesc& Desc, const char* Key)
{
	...
	if (Created == nullptr)
	{
		MAHO_LOG_CORE_ERROR("FRHIResourceManager::AcquireTexture: CreateTexture failed");
		return nullptr;
	}
...
FRHIBuffer* FRHIResourceManager::AcquireBuffer(const FRHIBufferDesc& Desc, const char* Key)
{
	...
	if (Created == nullptr)
	{
		MAHO_LOG_CORE_ERROR("FRHIResourceManager::AcquireBuffer: CreateBuffer failed");
		return nullptr;
	}
...
void FRHIResourceManager::Release(FRHIResource* Resource, bool bImmediate)
{
	...
	{
		MAHO_LOG_CORE_ERROR("FRHIResourceManager::Release: RefCount already 0 ({})", Resource->GetTypeName());
		return;
	}
