...
类型化资源的数据基类——只携带路径，派生类持有载荷。用户把 `TResourceImporter<T>` 特化成"字节 → 资源"的解码器。**以 `MAHO_RESOURCE_API` 导出为 DLL 接口**：实例可能比触发导入的调用方（被卸载的子插件）活得更久，导出后消费者只能经导入表引用 vtable 与删除析构，编译器无法把将死模块的 vptr 写进实例（具体资产类型同理标 `MAHO_ASSET_API`）。
