...
FAssetRegistry* GAssetRegistry = nullptr;
...
void FAssetRegistry::Scan(const std::filesystem::path& ContentDir, std::string_view MountAlias)
{
	...
	Paths::GetPaths()->SetRoot(MountAlias, ContentDir);
...
std::filesystem::path FAssetRegistry::Resolve(const FAssetPath& Path) const
{
	...
	return Paths::GetPaths()->Resolve(P);
}
...
extern "C" MAHO_ASSET_API Maho::FLayerBase* CreateLayer()
{
	return Maho::Asset::FAssetRegistry::CreateLayer();
}
