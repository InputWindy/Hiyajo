...
bool FVulkanMemoryAllocator::Initialize(VkInstance Instance, VkPhysicalDevice PhysicalDevice, VkDevice Device)
{
	...
	{
		MAHO_LOG_CORE_ERROR("FVulkanMemoryAllocator::Initialize: vmaCreateAllocator failed ({})", static_cast<int>(Result));
		Allocator = nullptr;
		return false;
	}

	MAHO_LOG_CORE_INFO("VMA allocator created");
	return true;
}
	...
	FRHIMemoryAllocation* OutOpaque)
{
	...
	if (Result != VK_SUCCESS)
	{
		MAHO_LOG_CORE_ERROR("FVulkanMemoryAllocator::CreateBuffer failed ({})", static_cast<int>(Result));
		return false;
	}
	...
	FRHIMemoryAllocation* OutOpaque)
{
	...
	if (Result != VK_SUCCESS)
	{
		MAHO_LOG_CORE_ERROR("FVulkanMemoryAllocator::CreateImage failed ({})", static_cast<int>(Result));
		return false;
	}
