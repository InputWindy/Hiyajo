...
// tag (TTool → FToolTag, TLayer → FLayerTag). A plugin that wants EXTRA markers
// appends them with FWithTags so the identity tag is kept.
//
// Tools are self-managed by design (plug-in-and-play) and need no extra marker;
// Layers are driven by the scheduler. FWithTags exists for future markers.
// ───────────────────────────────────────────────────────────────────────
