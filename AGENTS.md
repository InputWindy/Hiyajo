...
- **Install into the collector whose stages you mount** (FLayerCollector): the install tree (`PluginManager.json` -> `GetChildren` + `InstallChildrenOf(GetName())`) puts each layer into the collector that drives it -- the host's, or a collector layer's own (FRender owns its features, FExampleWorld its systems). A layer expands into a node only for the stages of *that* graph's stage list, so a layer installed into a collector whose stage set it does not mount contributes no nodes at all: it is never driven, its declared dependencies are never validated, and it silently sits there (the dependency check cannot even see it). The mirror-image mistake is a layer type installed into two collectors: the DLL is mapped once (LoadLibrary shares one copy and only bumps a refcount), but there are two layer INSTANCES -- two ImGui contexts, two of whatever the layer owns -- which is legal-but-rarely-intended; the per-collector name check cannot see it, so it is caught by reading the teardown trace, not by the loader.
