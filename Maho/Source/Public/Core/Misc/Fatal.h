 ...
 * Safe before FEngineBase/Log exist. Do not call from ordinary gameplay — use for contract failures only.
 */
[[noreturn]] MAHO_API void ReportFatal(const char* Message);

/** Install std::terminate handler once (call from process entry before CreateEngine). */
