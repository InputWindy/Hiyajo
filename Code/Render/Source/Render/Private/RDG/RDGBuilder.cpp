...
{
	if (Data == nullptr || Size == 0 || DstBuffer == nullptr)
	{
		return;
	}
...
void FRDGBuilder::CollectResourceLifetimes()
{
	...
	for (auto& Res : OwnedResources)
	{
		...
		if (R->IsExternal())
		{
			LT.FirstUse = 0; LT.LastUse = UINT32_MAX;
		}
		else
		{
			LT.FirstUse = UINT32_MAX; LT.LastUse = 0;
		}
		Lifetimes[R] = LT;
	}
	for (std::size_t i = 0; i < Passes.size(); ++i)
	{
		...
		for (const auto& Acc : Pass->GetReads())
		{
			...
			{
				if (Idx < It->second.FirstUse)
				{
					It->second.FirstUse = Idx;
				}
				if (Idx > It->second.LastUse)
				{
					It->second.LastUse = Idx;
				}
			}
		}
	...
	std::uint32_t& OutWidth, std::uint32_t& OutHeight)
{
	...
	if (Params == nullptr || Params->RenderTargets.empty())
	{
		return;
	}
	...
	{
		if (RT.Texture == nullptr || RT.Texture->GetRHI() == nullptr)
		{
			continue;
		}
...
void FRDGBuilder::ExecutePassesAndTransitions()
{
	...
	for (FCompiledPass& CP : CompiledPasses)
	{
		...
		if (Pass == nullptr || !Pass->GetExecute())
		{
			continue;
		}
