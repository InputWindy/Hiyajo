...
// The host: an assembly (IAssembly → Main) that ALSO assembles its plugin
// dependencies (TExtension = the dependency table). NOT a singleton — the
// host can be loaded dynamically and instantiated many times.
class F{name}
	: public Maho::TExtension<{name_upper}_EXTENSIONS>
	, public virtual Maho::IAssembly
{{
public:
...
def codegen_plugin_extensions(cproject_path: Path) -> Path:
	...
	extensions = ", ".join([f"{name}::F{name}"] + [f"Maho::{p}::F{p}" for p in plugins])
	...
	if full_includes:
		text += (
			...
			f"{name.upper()}_INCLUDE_PLUGINS to pull them in (they need complete\n"
			"// types to call T::Get().ExecuteStage etc.).\n"
...
def create_plugin(
	...
	(public / f"{plugin_name}.h").write_text(
		...
		f"class F{plugin_name} : public Maho::TExtensionList<F{plugin_name}>\n"
		f"{{\n"
		f"public:\n"
		f"\t// A plugin is a plain singleton — no Main, no IAssembly. Only\n"
		f"\t// applications (create project) inherit IAssembly and export\n"
		f"\t// CreateExtension.\n"
		f"}};\n\n"
		f"}} // namespace {plugin_name}\n\n"
		f"}} // namespace Maho\n",
	...
	(private / f"{plugin_name}.cpp").write_text(
		...
		f"namespace Maho\n{{\n\n"
		f"namespace {plugin_name}\n{{\n\n"
		f"// TODO: the plugin's logic (its ExecuteStage / interface methods).\n\n"
		f"}} // namespace {plugin_name}\n\n"
		f"}} // namespace Maho\n",
		encoding="utf-8", newline="\n",
	)
