...
def create_project(
	...
	# The project root = the entry plugin (an FEngineBase, generated by create_plugin).
...
def create_plugin(
	...
	kind: str = "feature",  # "entry" (FEngineBase root) | "feature" (FEngineLayer) | "engine" (pure lib)
) -> Path:
	"""
 ...
 plugin structure: .cplugin + .cmake
	...
	kind selects the class skeleton:
	  - "entry"   → F{name} : FEngineBase (MAHO_DECLARE_ENGINE + CreateEngine bridge)
	...
	if kind == "entry":
		# The application root — an FEngineBase owning the main loop. Exports CreateEngine.
		header_body = (
			f"#pragma once\n\n"
			...
			f"{child_includes}\n"
			f"namespace Maho\n{{\n\n"
			f"// {plugin_name} — the application root (an FEngineBase). Explicitly\n"
			f"// orchestrates feature lifecycles in order (no hidden recursion).\n"
			f"class F{plugin_name} : public FEngineBase\n"
			f"{{\n"
