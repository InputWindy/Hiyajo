...
def create_project(
	...
	# The project root = the entry plugin (an IEngine, generated by create_plugin).
	# The selected plugins' headers are #included (compile-level); the engine
	# anchors them by loading their DLLs. All project logic lives in features
	# (FEngineLayer nodes) driven by the entry's main-loop pipeline.
	create_plugin(
		project_name, engine_root, description=description,
		plugins_dir=project_dir / "Plugins", children=selected, kind="entry",
	)
...
def create_plugin(
	...
	kind: str = "feature",  # "entry" (IEngine root) | "feature" (FEngineLayer) | "engine" (pure lib)
) -> Path:
	"""
	Scaffold a new self-contained plugin under plugins_dir (default
 plugin structure: .cplugin + .cmake
	+ settings.json + AGENTS.md + docs + Source skeleton.

	kind selects the class skeleton:
	  - "entry"   → F{name} : IEngine (MAHO_DECLARE_ENGINE + CreateEngine bridge)
	  - "feature" → F{name} : FEngineLayer (BeginFrame/Tick/EndFrame stages)
	  - "engine"  → namespace Maho::{name} (pure library, no lifecycle)
	"""
	...
	# ── class skeleton per kind ────────────────────────────────────────────
	if kind == "entry":
		# The application root — an IEngine owning the main loop. Exports CreateEngine.
		header_body = (
			f"#pragma once\n\n"
			...
			f"{child_includes}\n"
			f"namespace Maho\n{{\n\n"
			f"// {plugin_name} — the application root (an IEngine). Explicitly\n"
			f"// orchestrates feature lifecycles in order (no hidden recursion).\n"
			f"class F{plugin_name} : public IEngine\n"
			f"{{\n"
			f"MAHO_DECLARE_ENGINE(F{plugin_name}, \"{plugin_name}.dll\");\n"
			f"\n"
			f"public:\n"
			f"\tvoid Initialize(int Argc, char** Argv) override;\n"
			f"\tint Main() override;\n"
			f"\tvoid Shutdown() override;\n"
			f"}};\n\n"
			f"}} // namespace Maho\n"
		)
		impl_body = (
			...
			f"namespace Maho\n{{\n\n"
			f"void F{plugin_name}::Initialize(int Argc, char** Argv)\n"
			f"{{\n"
			f"\t(void)Argc; (void)Argv;\n"
			f"\t// FLog::Get().Initialize(Argc, Argv);\n"
			f"\t// Install(&SomeFeature);\n"
			f"}}\n\n"
			f"int F{plugin_name}::Main()\n"
			f"{{\n"
			f"\treturn 0;\n"
			f"}}\n\n"
			f"void F{plugin_name}::Shutdown()\n"
			f"{{\n"
			f"\t// FLog::Get().Shutdown();\n"
			f"}}\n\n"
			f"}} // namespace Maho\n\n"
			f"// The C export the host (EntryPoint) looks up BY SYMBOL NAME.\n"
			f'extern "C" MAHO_{export}_API Maho::IEngine* CreateEngine()\n'
			f"{{\n"
			f"\treturn Maho::F{plugin_name}::CreateEngine();\n"
			f"}}\n"
		)
	elif kind == "engine":
		# Pure library — namespace-scoped, no lifecycle, no exported factory.
		header_body = (
			f"#pragma once\n\n"
			f'#include "{plugin_name}Api.h"\n'
			f"#include <Maho.h>\n"
			...
			f"// {plugin_name} — a pure library (no lifecycle, no singleton).\n"
			f"// Free functions / classes only; add your API here.\n\n"
			f"}} // namespace {plugin_name}\n"
			f"}} // namespace Maho\n"
		)
		impl_body = (
