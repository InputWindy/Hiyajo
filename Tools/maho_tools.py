...
def create_project(
	...
	# The selected plugins' headers are #included (compile-level); the FLayer<...>
	# drive list + ordering deps are user-written in the entry plugin's header.
...
def create_plugin(
	...
	(public / f"{plugin_name}.h").write_text(
		...
		f"{child_includes}\n"
		f"namespace Maho\n{{\n\n"
		f"// {plugin_name} — a Layer node.\n"
		f"//   FLayer<FChildA, FChildB, ...>  — hand-written: WHO this layer drives.\n"
		f"//   MAHO_EXTEND_DEPS(...)          — hand-written: ordering deps (optional).\n"
