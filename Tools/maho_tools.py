...
def create_project(
	...
	# The project root = the entry plugin (a FLayer, generated by create_plugin).
	# It INHERITS the single selected parent Layer (the project's base class).
	# All project logic is a layer; the EntryPoint exe merely loads its DLL.
	parent_layer = selected[0] if len(selected) == 1 else ""
	create_plugin(
		project_name, engine_root, description=description,
		plugins_dir=project_dir / "Plugins", parent_layer=parent_layer,
	)
...
def create_plugin(
	...
	parent_layer: str = "",  # base class the plugin's F{name} derives from (a layer)
) -> Path:
	"""
	...
	(public / f"{plugin_name}.h").write_text(
		...
		f"// the DLL factory + module path come from MAHO_DECLARE_LAYER. Install and\n"
		f"// uninstall hooks are spelled OUT here — you write the lifecycle logic.\n"
		f"class F{plugin_name}\n"
		+ (f"\t: public Maho::F{parent_layer}\n" if parent_layer else f"\t: public FLayer<>\n")
		+ f"\t, public IPlugin<IMain, IExit>\n"
		f"{{\n"
		f"MAHO_DECLARE_LAYER(F{plugin_name}, \"{plugin_name}.dll\");\n"
		f"\n"
		f"\t// The run entry — a layer that OWNS a loop writes while/tick/exit here;\n"
		f"\t// a driven layer returns after one frame of work.\n"
		f"\tint Main() override {{ return 0; }}\n"
		f"\tvoid Exit() override {{}}\n"
		f"\n"
		f"\t// Install what this layer manages (recursive). Args are the launch ones.\n"
		f"\tvoid Initialize(int Argc, char** Argv) override {{ (void)Argc; (void)Argv; }}\n"
