...
static_assert(
	Maho::FAssemblyExport<F{name}Engine>,
...
Maho::IAssembly* F{name}Engine::CreateExtension()
{{
	return new F{name}Engine();
}}

int F{name}Engine::Main(int Argc, char** Argv)
{{
	(void)Argc;
	(void)Argv;
...
{{
	...
	{{
		return {name}::F{name}Engine::CreateExtension();
	}}
}}
"""
...
def codegen_plugin_extensions(cproject_path: Path) -> Path:
	...
	# Class = F{Name}{Suffix}; the project's own engine root is F{Name}Engine.
	...
	text = (
		...
		f"#define {name.upper()}_SELF {name}::F{name}Engine\n"
