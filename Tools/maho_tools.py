...
# /FS serializes writes to ONE project's .pdb across cl.exe processes. The *_SubPlugins
# POST_BUILD step can nest an msbuild run beside the outer one (see
# Tools/build_subplugins.cmake), which otherwise puts two compilers on the same .pdb and
# dies with C1041 -- and leaves half-built DLLs in Binaries/<Config>.
if(MSVC)
	...
	for name in names:
		...
		if sub_plugins:
			...
			gen_lines += [
				...
				f"# It carries no sources and no output of its own, so it is parked under",
				f"# ThirdParty/CodeGen — a code-gen artifact of {name}, not a plugin of it.",
				f"add_custom_target({handle})",
				f"add_dependencies({handle} {' '.join(sub_plugins)})",
				f'set_target_properties({handle} PROPERTIES FOLDER "ThirdParty/CodeGen")',
...
def _write_cmake_lists(
	...
	# BuildType (default Runtime): a Runtime build excludes Type=Editor plugins so
	# the release/packaged product never carries the editor. This is the single
	# selection filter — chain resolution, plugin targets, inclusion dirs and the
	# host entry deps all derive from `selected`, so Editor DLLs never build.
	build_type = cproject_data.get("BuildType", "Runtime")
	...
	engine_chain = [
		p for p in chain
		if infos.get(p, {}).get("public_dir", "").startswith("${ENGINE_DIR}")
	]
...
def read_cplugin(path: Path) -> dict[str, Any]:
	...
	# Plugin build category: Runtime (default) | Editor. Editor-typed plugins are
	# excluded from a default (Runtime) build via .cproject BuildType filtering.
	plugin_type = data.get("Type", "Runtime")
	if not isinstance(plugin_type, str) or not plugin_type.strip():
		plugin_type = "Runtime"
	plugin_type = plugin_type.strip()
	if plugin_type not in ("Runtime", "Editor"):
		raise ValueError(
			f"Invalid .cplugin (Type must be Runtime|Editor, got {plugin_type!r}): {path}"
		)
