...
# The engine third-parties are the shared infra (glm, nlohmann/json, …); PUBLIC
# so every Consumer (entry plugin + EntryPoint + sub-plugins) can just
# #include <glm/glm.hpp> / <nlohmann/json.hpp>.
...
def create_project(
	...
	# The selected mounted plugin becomes its child (code-gen into FLayer<...>).
