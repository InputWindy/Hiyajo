...
// The project root = a Layer: an installable application node. It inherits the
// root (FLayerBase), the parallel scheduler, module loading, the child instance
// array, and the standard lifecycle (IInit/IMain/IShutdown). It implements the
// interfaces chosen at creation (IPlugins), schedules the layers chosen at
// creation (Layers), and is the one place the user writes the main loop / exit.
// NOT a singleton — the root can be loaded dynamically and instantiated many times.
class F{name}Layer
	: public Maho::FLayer<{LAYERS}>
	, public Maho::IPlugin<{IPLUGINS}>
{{
public:
	/** The assembly factory — the ONLY way an FLayerBase root is created. */
	static Maho::FLayerBase* CreateExtension();

	int Main() override;

	// TODO: drive your schedule — the loop lives HERE so the layer owns exit:
	//
	//   bool bExit = false;
	//   int Main() override
	//   {{
	//       while (!bExit)
	//       {{
	//           Query().Select<IMain>().ForEach([](IMain& C) {{ C.Main(); }});
	//       }}
	//       return 0;
	//   }
}};
...
Maho::FLayerBase* F{name}Layer::CreateExtension()
{{
	return new F{name}Layer();
}}

int F{name}Layer::Main()
{{
	return 0;   // TODO: your main loop — the layer owns exit
}}
...
extern "C"
{{
	MAHO_GAME_EXPORT Maho::FLayerBase* CreateExtension()
	{{
		return {name}::F{name}Layer::CreateExtension();
	}}
}}
"""
...
def create_project(
	...
	(plugin_public / f"{project_name}.h").write_text(
		PUBLIC_HEADER.format(
			name=project_name,
			name_upper=project_name.upper(),
			LAYERS="",       # the project root's children — hand-written layers
			IPLUGINS="",     # the root's outward interfaces — hand-written
		),
		encoding="utf-8", newline="\n",
	)
...
def create_plugin(
	...
	(public / f"{plugin_name}.h").write_text(
		...
		f"// you want; the root (FLayerBase), scheduler, module loading, lifecycle\n"
		f"// (IInit/IMain/IShutdown) and the child array all come from FLayer.\n"
		...
		f"\n"
		f"\t// Your loop lives HERE so this layer owns exit (IExit when you need it).\n"
		f"\tint Main() override {{ Query().Select<IMain>().ForEach([](IMain& C) {{ C.Main(); }}); return 0; }}\n"
		f"}};\n\n"
		f"}} // namespace Maho\n",
		encoding="utf-8", newline="\n",
	)
