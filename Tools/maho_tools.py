...
static_assert(
	Maho::FAssemblyExport<F{name}Layer>,
...
Maho::IAssembly* F{name}Layer::CreateExtension()
{{
	return new F{name}Layer();
}}

int F{name}Layer::Main(int Argc, char** Argv)
{{
	(void)Argc;
	(void)Argv;
...
def codegen_plugin_extensions(cproject_path: Path) -> Path:
	...
	extensions = ", ".join(f"Maho::{_class_of(p)}" for p in plugins)
	...
	text = (
		...
		f"#define {name.upper()}_SELF {name}::F{name}Layer\n"
...
def create_plugin(
	...
	(public / f"{plugin_name}.h").write_text(
		...
		f"#include <Engine/Tool.h>\n\n"
		f"namespace Maho\n{{\n\n"
		f"namespace {plugin_name}\n{{\n\n"
		f"// A tool plugin: a dependency table + a singleton (NOT an assembly).\n"
		f"class F{plugin_name} : public Maho::TTool<F{plugin_name}>\n"
		f"{{\n"
		f"public:\n"
		f"\t// The plugin's capabilities (callers call them directly via\n"
		f"\t// F{plugin_name}::Get(); no scheduler ownership).\n"
		f"}};\n\n"
		f"}} // namespace {plugin_name}\n\n"
		f"}} // namespace Maho\n",
		encoding="utf-8", newline="\n",
