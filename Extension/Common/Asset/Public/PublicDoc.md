...
| `FAssetRegistry : TExtensionList<FAssetRegistry>` | 注册表单例（无 Main/IAssembly） |
| `FAssetRegistry::ExecuteStage(EAssetStage)` | 阶段分发（Init / Shutdown 清空） |
| `Scan(ContentDir, MountAlias)` | 递归索引内容目录 |
| `Find(Path)` | 逻辑路径查元数据，缺失返回 nullptr |
