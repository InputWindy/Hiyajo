...
namespace
{
	...
	public:
		...
		[[nodiscard]] bool WantsClose() const { return Window != nullptr && glfwWindowShouldClose(Window); }
...
bool FPlatformTool::CreateWindow(int Width, int Height, std::string_view Title)
{
	DestroyWindow();
	FPlatformBackend Backend = CreateWindowBackend(Width, Height, Title);
	Surface = std::move(Backend.Surface);
	EventPump = std::move(Backend.EventPump);
	CloseQuery = std::move(Backend.CloseQuery);
	return Surface != nullptr && Surface->GetNativeWindow() != nullptr;
}

bool FPlatformTool::CreateHeadlessContext(int Width, int Height)
{
	DestroyWindow();
...
void FPlatformTool::DestroyWindow()
{
	Surface.reset();
	EventPump = {};
	CloseQuery = {};
...
FNativeSurface FPlatformTool::GetNativeWindow() const
{
	return Surface != nullptr ? Surface->GetNativeWindow() : nullptr;
}

bool FPlatformTool::ShouldClose() const
{
	return CloseQuery && CloseQuery();
}
