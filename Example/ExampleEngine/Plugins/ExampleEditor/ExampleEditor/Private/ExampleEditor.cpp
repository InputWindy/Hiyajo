...
namespace
{
	// Relocated data-driven control dispatcher: one branch per EUIControlType. The game's
	// UISystem no longer owns this -- the EDITOR takes over the full control set. Each
	// control is rendered into the editor's own ImGui context; interaction uses an
	// immediate-mode toggle (no event write-back yet -- a real editor would bind a
	// selection/action here).
	void DrawEditorControl(const Maho::GameWorld::FUIControl& C)
	{
		switch (C.Type)
		{
		case Maho::GameWorld::EUIControlType::Label:
			ImGui::TextUnformatted(C.Text.empty() ? "<label>" : C.Text.c_str());
			break;
		case Maho::GameWorld::EUIControlType::Text:
		{
			char Buf[256];
		...
		case Maho::GameWorld::EUIControlType::Image:
			// Image controls are drawn by the viewport / worker (SceneColor mirror);
			// here just label it.
			ImGui::TextUnformatted("[image]");
			break;
		case Maho::GameWorld::EUIControlType::Button:
			if (ImGui::Button(C.Text.empty() ? "<button>" : C.Text.c_str()))
			{
				// Wire an editor action (select / toggle) here.
			}
			break;
		case Maho::GameWorld::EUIControlType::Slider:
		{
			float V = C.V0;
			if (ImGui::SliderFloat("##slider", &V, C.V0, C.V1))
			{
				// V is the new slider value.
			}
			break;
		}
		case Maho::GameWorld::EUIControlType::Checkbox:
		{
			bool V = C.V0 > 0.5f;
			if (ImGui::Checkbox("##check", &V))
			{
				// V is the new state.
			}
			break;
		}
		case Maho::GameWorld::EUIControlType::Separator:
			ImGui::Separator();
			break;
		}
	}

	// Small hardcoded editor control set (relocated from the game's UISystem demo).
	const std::vector<Maho::GameWorld::FUIControl>& EditorSampleControls()
	{
		static const std::vector<Maho::GameWorld::FUIControl> Controls = {
			{ Maho::GameWorld::EUIControlType::Label, "Editor controls" },
			{ Maho::GameWorld::EUIControlType::Separator },
			{ Maho::GameWorld::EUIControlType::Button, "Rebuild" },
			{ Maho::GameWorld::EUIControlType::Slider, "", 0, 0, 0.5f, 1.0f },
			{ Maho::GameWorld::EUIControlType::Checkbox, "Wireframe", 0, 0, 1.0f, 1.0f },
		};
		return Controls;
	}
}
...
void FExampleEditor::InitViews(FRender& R)
{
	...
	// Build the editor UI directly through this feature's context -- the editor owns its
	// whole UI (no external UIBuilder). The SceneColor mirror (R.GetMirror, keyed
	// "SceneColor") is the viewport background when the scene feature is installed.
	const bool bSceneReady = (Scene::GetScene() != nullptr);
	BuildEditorUI(R, bSceneReady);
...
void FExampleEditor::BuildEditorUI(FRender& R, bool bSceneReady)
{
	// Docking host: a fullscreen dockspace behind the editor windows.
	...
	// -- Viewport window: sample the SceneColor mirror as the scene background.
	ImGui::Begin("Viewport");
	{
		const std::uint32_t TextureId = Name::FName("SceneColor").GetId();
		const ImVec2 Avail = ImGui::GetContentRegionAvail();
		if (bSceneReady && TextureId != 0)
		{
			// ImTextureID carries the SceneColor mirror FName id; the translation above
			// resolves it to the RDG texture (pooled sampler) for the per-batch set.
			ImGui::Image(static_cast<ImTextureID>(TextureId), Avail);
		}
		else
		{
			ImGui::TextUnformatted("Scene not ready");
		}
	}
	ImGui::End();

	// -- Outliner (sample world entity list; a real editor reads FGameWorld's registry).
	ImGui::Begin("Outliner");
	{
		static const char* Names[] = { "Root", "CameraActor", "DirectionalLight", "Player" };
		for (const char* N : Names)
		{
			if (ImGui::Selectable(N))
			{
				// select the entity (placeholder).
			}
		}
	}
	ImGui::End();

	// -- Inspector (placeholder for the selected entity's properties).
	ImGui::Begin("Inspector");
	{
		ImGui::TextUnformatted("Selected: camera");
		ImGui::Separator();
		DrawEditorControl({ GameWorld::EUIControlType::Label, "Transform" });
		DrawEditorControl({ GameWorld::EUIControlType::Slider, "", 0, 0, 0.f, 10.f });
		DrawEditorControl({ GameWorld::EUIControlType::Checkbox, "bVisible", 0, 0, 1.f, 1.f });
	}
	ImGui::End();

	// -- Controls: the relocated data-driven control set (DrawControl dispatcher).
	ImGui::Begin("Controls");
	{
		const auto& Controls = EditorSampleControls();
		for (const GameWorld::FUIControl& C : Controls)
		{
			DrawEditorControl(C);
		}
	}
...
// The C export FRender / the host looks up BY SYMBOL NAME for dynamic install.
